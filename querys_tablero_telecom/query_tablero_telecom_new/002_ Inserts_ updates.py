...
actualizacion = '23/10/2020'
#PERIODO DE LA SEMANA HABIL A ACTUALIZAR
descripcion = '19/10 a 23/10'  

#0001 primer insert
cur.execute("INSERT INTO datos.tablero_transporte_(id_cuadricula, num_semana_actual,cant_lineas_actual) select fraccion, a.semana, round(median (a.moviles::int)) as n_lineas from datos.dispositivos_por_grilla a left  join aux.calendario_2020 c using (fecha) where c.dia_semana NOT IN (6,7,8)  and a.semana= " +
...
cur.execute("update datos.tablero_transporte_ a set num_semana_precuarentena = 2 where num_semana_precuarentena is null and num_semana_actual = "+str(semana_actual))
connection.commit()
cur.execute("update  datos.tablero_transporte_1 a set cant_lineas_precuarentena= b.n_lineas from (select fraccion, a.semana, round(median (a.moviles::int)) as n_lineas from datos.dispositivos_por_grilla a left  join aux.calendario_2020 c using (fecha) where c.dia_semana NOT IN (5,6,7,8)  and hora between '10:00' and '17:00' and  c.semana= 2 group by fraccion,a.semana)b where a.id_cuadricula=b.fraccion	 and num_semana_actual = "+str(semana_actual)+" and  a.cant_lineas_precuarentena is null")
connection.commit()
print(datetime.datetime.today())
...
cur.execute("update datos.tablero_transporte_ a set num_semana_anterior = "+str(semana_anterior)+" where semana_anterior is null and num_semana_actual = "+str(semana_actual))
connection.commit()
cur.execute("update datos.tablero_transporte_ a set cant_lineas_semana_anterior = b.n_lineas from (select fraccion, a.semana, round (median (a.moviles::int)) as n_lineas from datos.dispositivos_por_grilla a left  join aux.calendario_2020 c using (fecha)where c.dia_semana NOT IN (5,6,7,8) and a.semana="+str(semana_anterior)+" and hora between '10:00' and '17:00' group by fraccion,a.semana) b where  fraccion= id_cuadricula and a.num_semana_anterior= '"+str(semana_anterior)+"' and num_semana_actual = "+str(semana_actual))
connection.commit()
print(datetime.datetime.today())
...
cur.execute("update  datos.tablero_transporte_ a set poblacion_caba= b.n_lineas from (select fraccion, a.semana,round(median (a.moviles::int)) as n_lineas from datos.dispositivos_por_grilla a left  join aux.calendario_2020 c using (fecha) where c.dia_semana NOT IN (5,6,7,8) and a.semana= '"+str(semana_actual)+"' and hora ='4:00'group by fraccion,a.semana)b where a.poblacion_caba is null and a.id_cuadricula=b.fraccion and b.semana="+str(semana_actual))
connection.commit()
print(datetime.datetime.today())
...
cur.execute("update datos.tablero_transporte_  a set actualizacion ='"+actualizacion+"',descripcion='"+descripcion+"' where num_semana_actual="+str(semana_actual))
connection.commit()
print(datetime.datetime.today())
print('Termine de actualizar la fecha y el periodo correspondiente a la semana')

print('Actualizo los valores en null o en 0 ')
cur.execute("update datos.tablero_transporte_ a set cant_lineas_semana_anterior = '1', cant_lineas_actual = '1'where cant_lineas_semana_anterior is null or cant_lineas_actual is null ")
connection.commit()
print(datetime.datetime.today())
